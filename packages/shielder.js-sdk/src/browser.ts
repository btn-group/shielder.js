...
import {
  SHIELDER_DEPOSIT_PK_BYTES_URL,
  SHIELDER_WITHDRAW_PK_BYTES_URL,
} from "./constants";
...
async function deposit(args: Deposit) {
  ...
  const depositWasmResult = await depositWasm(
    JSON.stringify(args),
    JSON.stringify({
...
async function withdraw(args: Withdraw) {
  ...
  const withdrawWasmResult = await withdrawWasm(
    JSON.stringify(args),
    JSON.stringify({
      nested: Array.from(flatten),
    }),
...
export { initWasm, deposit, withdraw, Deposit, Withdraw };
